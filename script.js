function Employee(name, age, department, salary) {
    ...
    let employees = [
    new Employee("Ram", 30, "Marketing", 50000),
    new Employee("Ankit", 35, "HR", 60000),
    new Employee("Bob", 28, "IT", 70000),
    new Employee("Rohit", 34, "Marketing", 55000)
    ];
    ...
    displayOutput(`Average Salary: <span>&#8377;</span>${averageSalary.toFixed(2)}`);
    }
    function findEmployeesByDepartment(department) {
    let filteredEmployees = employees.filter(emp => emp.department === department);
    displayOutput(`Employees in ${department}: ${filteredEmployees.map(emp =>
    emp.name).join(', ')}`);
    }
